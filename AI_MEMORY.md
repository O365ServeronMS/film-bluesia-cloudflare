...
- `HlsVideo.tsx`: HTML5 video player for OPhim direct m3u8 streams, with hls.js-first playback on MSE-capable browsers, native HLS fallback, conservative/adaptive buffer tuning, retry config, fatal error recovery, dynamic manifest quality selection, and local subtitle upload.
...
- OPhim HLS player UI rule: do not show production hls.js readiness/debug/status badges such as `Sẵn sàng phát bằng hls.js`. Keep the top-right quality selector backed by `hls.levels` (`Tự động` maps to `currentLevel = -1`, manual options map to manifest level indexes) and keep the yellow local subtitle upload button. The upload path accepts `.vtt` directly, converts `.srt` to WebVTT, and revokes replaced/unmounted subtitle ObjectURLs.
